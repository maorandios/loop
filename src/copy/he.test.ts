...
describe("cloud error mapping", () => {
  ...
  it("includes the Milestone 5 Hebrew states", () => {
    ...
    expect(he.primaryAction).toBe("פעולה");
    expect(he.primaryInfo).toBe("ידיעה");
    expect(he.primaryCompleted).toBe("הושלם");
    expect(he.noPrimaryAction).toBe("אין בקשות לפעולה");
    expect(he.noPrimaryInfo).toBe("אין בקשות לידיעה");
    expect(he.noPrimaryCompleted).toBe("אין בקשות שהושלמו");
    expect(he.searchRequests).toBe("חיפוש בבקשות");
    expect(he.clearSearch).toBe("נקה חיפוש");
    expect(he.noSearchMatches).toBe("לא נמצאו בקשות מתאימות");
    expect(he.unreadRequests).toBe("יש בקשות חדשות");
    ...
    expect(he.fromBy).toBe("מאת");
    expect(he.activity).toBe("היסטוריית פעילות");
    expect(he.actions).toBe("פעולות");
    expect(he.backToMenu).toBe("חזרה לתפריט");
    expect(he.approve).toBe("אישור");
    expect(he.reject).toBe("דחיה");
    expect(he.attachFile).toBe("צירוף קובץ");
    expect(he.dropHint).toBe("גרור קובץ לכאן");
    expect(he.browseFromExplorer).toBe("פתיחת סייר הקבצים");
    expect(he.sendReminder).toBe("שליחת תזכורת");
  ...
  it("includes the Milestone 6 transfer copy", () => {
    ...
    expect(he.requestTypeLabel).toBe("סוג בקשה");
    expect(he.kindReview).toBe("בדיקה");
    expect(he.kindUpdate).toBe("עדכון");
    expect(he.removeFile).toBe("הסרת קובץ");
